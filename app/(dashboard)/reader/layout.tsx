    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'#0c0d10', fontFamily:"'Syne',system-ui,sans-serif" }}>
      <aside style={{ width:220, minWidth:220, background:'#131520', borderRight:'1px solid #252840', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <div style={{ padding:'16px 20px', borderTop:'1px solid #252840' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:'50%', background:'linear-gradient(135deg,#5ba4f5,#3dd6a3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color:'#fff', flexShrink:0 }}>
              {user.name?.slice(0,2).toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize:13, fontWeight:600, color:'#e8eaf8' }}>{user.name}</div>
              <div style={{ fontSize:10, color:'#5ba4f5', fontFamily:"'JetBrains Mono',monospace" }}>Reader</div>
            </div>
          </div>
        </div>
      </aside>
